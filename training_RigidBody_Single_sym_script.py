...
#==============================================================================
fl = 'DynamicalSystems/SavedTrainingData/RigidBody/Single/'
npz_file = np.load(fl + 'TDRigidBodySingleN120M40Tau01.npz')
...
for L in [2]:
    ...
    for m in [16, 32]:
        ...
        for k in range(2):
            ...
            plt.xticks([1, 10, 10**2, 10**3, 10**4, 10**5])
            plt.yticks([10**(-9), 10**(-8), 10**(-7), 10**(-6), 10**(-5), 10**(-4), 10**(-3)])
            ax.set_title('SymLocSympNets, Rigid body: L=' + str(L) + 
                         ', m=' + str(m) + ', k=' + str(k))
            # optional: save figure
            plt.savefig('Figures/RigidBody/Single/LossAcc/sym_LossAcc_L' + str(L) + 
                        'm' + str(m) + 'k' + str(k) + '.png', 
            ...
            if sch:
                ...
                file_w = g + "sym_RigidBodySingleN120M40Tau01Epoch200TH" + f
            model = model.to('cpu')    
