"""
Script file for training symmetric locally-symplectic neural networks 
...
#==============================================================================
fl = 'DynamicalSystems/SavedTrainingData/RigidBody/Whole/'
npz_file = np.load(fl + 'TDRigidBodyWholeN300M100Tau01.npz')
...
for L in [2]:
    ...
    for m in [32, 64]:
        ...
        for k in range(2):
            ...
            ax.axis([1, epochs, 10**(-10), 10**(-3)])
            plt.xticks([1, 10, 10**2, 10**3, 10**4, 10**5, 10**6])
            plt.yticks([10**(-10), 10**(-9), 10**(-8), 10**(-7), 10**(-6), 
                        10**(-5), 10**(-4), 10**(-3)])
            ax.set_title('SymLocSympNets, Rigid body: L=' + str(L) + 
                         ', m=' + str(m) + ', k=' + str(k))
            # optional: save figure
            plt.savefig('Figures/RigidBody/Whole/LossAcc/sym_LossAcc_L' + str(L) + 
                        'm' + str(m) + 'k' + str(k) + '.png', 
            ...
            if sch:
                file_w = g + "sym_schRigidBodyWholeN300M100Tau01Epoch1000TH" + f
            else:
                file_w = g + "sym_RigidBodyWholeN300M100Tau01Epoch1000TH" + f
            model = model.to('cpu') 
